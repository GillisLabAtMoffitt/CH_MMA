---
title: "Investigate MMA association with CH no indel in Multiple Myeloma"
...
<span style="color: white;">MMA in Multiple Myeloma</span>
...
We had 5 patients with tumor samples contamination that were removed from the following analyses.
```{r load}
...
Total pop is `r nrow(ch_wide)`, % are calculated over CH patients only here `r nrow(ch_wide %>% filter(CH_status_no_indel == "CH"))`.
```{r CH genes}
ch_wide %>% 
...
`r nrow(ch_wide %>% filter(CH_status_no_indel == "CH"))` CH patients - `r nrow(ch_wide %>% filter(CH_status_no_indel == "CH" & DNA_damage_class == "Yes"))` with DNA damages genes
```{r by DNA damages}
ch_wide %>%
  select(#MMA_MMdx_results, 
         MMA_germline_results,
...
`r nrow(ch_wide)` total patients - `r nrow(ch_wide %>% filter(CH_status_no_indel == "CH" & DNA_damage_class == "Yes"))` CH patients with DNA damages genes, `r nrow(ch_wide %>% filter(is.na(CH_status_no_indel)))` Unknown CH,  
- `r nrow(ch_wide %>% filter(CH_status_no_indel == "CH" & DNA_damage_class == "No"))` CH / No damages genes + `r nrow(ch_wide %>% filter(CH_status_no_indel == "No_CH"))` no CH = `r nrow(ch_wide %>% filter(CH_status_no_indel == "No_CH" | DNA_damage_class == "No"))` patients.
```{r}
ch_wide %>%
...
`r nrow(ch_wide %>% filter(CH_status_no_indel == "CH"))` CH - `r nrow(ch_wide %>% filter(CH_status_no_indel == "CH" & TD_class == "Yes"))` with TD genes
```{r by TD}
ch_wide %>%
  select(#MMA_MMdx_results, 
         MMA_germline_results,
...
`r nrow(ch_wide)` total patients - `r nrow(ch_wide %>% filter(CH_status_no_indel == "CH" & TD_class == "Yes"))` CH patients with TD genes, `r nrow(ch_wide %>% filter(is.na(CH_status_no_indel)))` Unknown CH,  
- `r nrow(ch_wide %>% filter(CH_status_no_indel == "CH" & TD_class == "No"))` CH / No TD genes + `r nrow(ch_wide %>% filter(CH_status_no_indel == "No_CH"))` no CH = `r nrow(ch_wide %>% filter(CH_status_no_indel == "No_CH" | TD_class == "No"))` patients.
```{r}
ch_wide %>%
...
tbl2 <- coxph(Surv(time = patients_clean_tumor_male$month_at_os, 
             event = patients_clean_tumor_male$os_event) ~ 
               MMA_germline_results_cat + Age_at_MMonly_diagnosis + raceeth + CrCl_interpretation + 
               bmi_germline_results + b12_germline_results + CH_status_no_indel, 
             data =  patients_clean_tumor_male) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
...
tbl1_1 <- glm(MMA_germline_results_cat ~ Age_at_MMonly_diagnosis + raceeth + CH_status,
             data = patients_clean_tumor) %>%
...
tbl2_1 <- glm(MMA_germline_results_cat ~ Age_at_MMonly_diagnosis + raceeth + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status,
             data = patients_clean_tumor) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
...
tbl1_2 <- glm(MMA_germline_results_cat ~ Age_at_MMonly_diagnosis + raceeth + CH_status,
             data = patients_clean_tumor_female) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
                         ) %>% 
  ...
  bold_labels() %>% bold_p(t = .05)
tbl2_2 <- glm(MMA_germline_results_cat ~ Age_at_MMonly_diagnosis + raceeth + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status,
             data = patients_clean_tumor_female) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
...
tbl1_3 <- glm(MMA_germline_results_cat ~ Age_at_MMonly_diagnosis + raceeth + CH_status,
             data = patients_clean_tumor_male) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
                         ) %>% 
  ...
  bold_labels() %>% bold_p(t = .05)
tbl2_3 <- glm(MMA_germline_results_cat ~ Age_at_MMonly_diagnosis + raceeth + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status,
             data = patients_clean_tumor_male) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
...
## Continuous
```{r}
tbl1_1 <- glm(MMA_germline_results ~ Age_at_MMonly_diagnosis + raceeth + CH_status,
             data = patients_clean_tumor) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
                         ) %>% 
           modify_spanning_header(everything() ~ paste("Association with MMA in overall data")) %>% 
  bold_labels() %>% bold_p(t = .05)
tbl2_1 <- glm(MMA_germline_results ~ Age_at_MMonly_diagnosis + raceeth + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status,
             data = patients_clean_tumor) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
...
tbl1_2 <- glm(MMA_germline_results ~ Age_at_MMonly_diagnosis + raceeth + CH_status,
             data = patients_clean_tumor_female) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
                         ) %>% 
           modify_spanning_header(everything() ~ paste("Association with MMA in female data")) %>% 
  bold_labels() %>% bold_p(t = .05)
tbl2_2 <- glm(MMA_germline_results ~ Age_at_MMonly_diagnosis + raceeth + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status,
             data = patients_clean_tumor_female) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
...
tbl1_3 <- glm(MMA_germline_results ~ Age_at_MMonly_diagnosis + raceeth + CH_status,
             data = patients_clean_tumor_male) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
                         ) %>% 
           modify_spanning_header(everything() ~ paste("Association with MMA in male data")) %>% 
  bold_labels() %>% bold_p(t = .05)
tbl2_3 <- glm(MMA_germline_results ~ Age_at_MMonly_diagnosis + raceeth + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status,
             data = patients_clean_tumor_male) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
