---
title: "Investigate MMA association with CH no indel in non Multiple Myeloma patients"
...
<span style="color: white;">MMA in non Multiple Myeloma patients</span>
...
```{r load}
non_HEM <- read_csv(paste0(here::here(), "/sample ids non-HEM v407 with MMA data.csv")) %>% 
  select(avatar_key, mrn, SLID_germline,
         dob, age_at_diagnosis, 
         germline_collection_date, 
         disease_type_conformed, histology)
MGUS_MMA <- read_csv(paste0(here::here(), "/sample ids MGUS v407 with MMA data.csv")) %>% 
  select(avatar_key, mrn, SLID_germline, 
         dob, age_at_diagnosis, 
         germline_collection_date, 
         disease_type_conformed, histology)

germline_patient_data <- bind_rows(non_HEM, MGUS_MMA) %>%
...
no_indel_CH_status <- 
  read_csv(paste0(path, "/data/CHcalls_woINDELS_ASXL10_11.15.22_list_Christelle.csv"), 
           col_select = c(patient_id, CH_status),
           n_max = 651) %>% 
  mutate(patient_id = str_remove(patient_id, "_normal"))

ch_genes <- read_csv(paste0(path, "/data/CHcalls_woINDELS_ASXL10_11.15.22_muts_Christelle.csv")) %>% 
  mutate(patient_id = str_remove(patient_id, "_normal")) %>% 
  filter(`...2` == "CH") %>% 
...
demo <- 
  readxl::read_xlsx(here::here("MMA_avatar_lab_report.xlsx"), 
                           sheet = "PTE Demographics") %>%
  select(MRN, Sex = GENDER_DERIVED_DESC, 
         ethnicity = ETHNICITY_DERIVED_DESC , race = RACE_DERIVED_DESC, 
         os_date = VITAL_STATUS_DT, os_event = DERIVED_VITAL_STATUS_DESC)
```

# MMA levels distribution in MM patients (No limit of time between germline and diagnosis)
...
germline_patient_data <- germline_patient_data %>%
  ...
  mutate(int = abs(interval(start = VITALS_DTM,
                            end = germline_collection_date) /
                     duration(n=1, units = "days"))) %>%
  group_by(mrn, germline_collection_date) %>%
  mutate(min = min(int)) %>%
...
germline_patient_data <- germline_patient_data %>%
  ...
  mutate(age_at_creatinine = round(interval(start = dob, end = creatinine_date)/
...
#   b12_value > 1999                         ~ 2500,
```
<!-- all the ">" in b12 goes into the high normalcy so we might need to put them all in the same group... -->
<!-- the b12 normalcy overlap when looking at the available value range. Maybe ask a pharmacist...HAHAHA -->
And we created our own classification:
```{r, echo=TRUE}
# b12_interpretation = case_when(
#     b12_germline_results < 200             ~ "Low",
#     b12_germline_results >= 200 &
#       b12_germline_results <= 900          ~ "Normal",
#     b12_germline_results > 900             ~ "High"
#   ))
```
...
germline_patient_data <- germline_patient_data %>%
  left_join(., MMA_2 %>%
              select(MRN, 
                     MMA_germline_results = LAB_RESULT, 
                     MMA_date = LAB_DATE),
            by = c("mrn" = "MRN")) %>%
...
patients_MMA <- germline_patient_data %>% 
  select(mrn, avatar_key, starts_with("height"), starts_with("weight"), 
...
germline_patient_data <- germline_patient_data %>% 
  ...
  mutate_at(c("MMA_germline_results"), 
            ~ case_when(
    str_detect(., "<")                 ~ 0.05,
    TRUE                               ~ as.numeric(.)
  ))
...
# I. Patients and lab summary

```{r summ table}
germline_patient_data %>% 
  mutate(Whole = "patients with blood") %>% 
  select(#MMA_MMdx_results, 
         MMA_germline_results, 
         creatinine_germline_results,
         calculated_CrCl,
         b12_germline_results, b12_interpretation,
  ...
  tbl_summary(by = Whole, 
              ...
              digits = list(age_at_diagnosis ~ 2)#,
              # missing = "no"
              ) %>% 
  bold_labels()
```

CrCl levels in our population
```{r}
germline_patient_data %>% 
  filter(!is.na(CrCl)) %>% 
  ggplot(aes(x= CrCl))+
...
MMA at germline levels in our population. The blue line represent the separation between low and high MMA categories (0.4).
```{r}
germline_patient_data %>% 
...
<!-- # III. MMA levels by CH status classes (DNA damages and TD) -->
...
<!--     TRUE                         ~ "No" -->
<!--   )) %>%  -->
<!--   right_join(., germline_patient_data, by = c("patient_id" = "SLID_germline")) -->
<!-- ``` -->
...
<!-- **Classify CH as mutations in DNA damage genes only. If no mutation in DNA damage genes, CH=no**   -->
...
<!-- ```{r MMA level and VAF} -->
<!-- ch_genes %>%  -->
<!--   full_join(., germline_patient_data, by = c("patient_id" = "SLID_germline")) %>%  -->
<!--   arrange(mrn, desc(VAF_normal)) %>%  -->
<!--   distinct(mrn, .keep_all = TRUE) %>%  -->
<!--   mutate(CH_status_no_indel = case_when( -->
...
<!-- ```{r} -->
<!-- ch_genes %>% -->
<!--   left_join(., germline_patient_data, by = c("patient_id" = "SLID_germline")) %>% -->
...
female <- germline_patient_data %>%
  filter(Sex == "Female") %>% 
  select(#MMA_MMdx_results, 
         MMA_germline_results,
         CH_status_no_indel) %>% 
  tbl_summary(by = CH_status_no_indel, 
              type = list(MMA_germline_results ~ "continuous"),
              label = list(CH_status_no_indel ~ "CH status wo indels"),
              sort = list(everything() ~ "frequency")) %>% 
  bold_labels() %>% add_p() %>% bold_p(t = .05)
...
<!--   ggplot(aes(x= Age_at_germcollect, y= MMA_germline_results))+ -->
<!--   geom_point()+ -->
<!--   geom_smooth(method = "lm") + -->
<!--   stat_cor(size=5, label.x = 40, label.y = 1.5)+ -->
<!--   stat_regline_equation(label.x = 65, label.y = 1.5, size=5)+ -->
<!--   theme_classic() -->
<!-- ``` -->
<!-- <br> -->
<!-- <br> -->
...
<!-- #                    y = (Surv(time = germline_patient_data$month_at_os,  -->
<!-- #                              event = germline_patient_data$os_event)), -->
<!-- #                    exponentiate = TRUE) %>%  -->
<!-- #   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") %>% -->
<!-- #   bold_labels() %>% italicize_levels() -->
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data$month_at_os,  -->
<!-- #              event = germline_patient_data$os_event) ~  -->
<!-- #                MMA_germline_results_cat + age_at_diagnosis + Sex + race, ethnicity + calculated_CrCl +  -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data$month_at_os,  -->
<!-- #              event = germline_patient_data$os_event) ~  -->
<!-- #                MMA_germline_results + age_at_diagnosis + Sex + race, ethnicity + calculated_CrCl +  -->
<!-- #                bmi_germline_results + b12_germline_results + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
...
<!-- tbl2 <- coxph(Surv(time = germline_patient_data$month_at_os,  -->
<!--              event = germline_patient_data$os_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + Sex + race, ethnicity + CrCl_interpretation +  -->
<!--                bmi_germline_results + b12_germline_results + CH_status_no_indel,  -->
<!--              data =  germline_patient_data) %>% -->
<!--   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data$month_at_os,  -->
<!-- #              event = germline_patient_data$os_event) ~  -->
<!-- #                MMA_germline_results + age_at_diagnosis + Sex + race, ethnicity + CrCl_interpretation +  -->
<!-- #                bmi_germline_results + b12_germline_results + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data$month_at_os,  -->
<!-- #              event = germline_patient_data$os_event) ~  -->
<!-- #                MMA_germline_results_cat + age_at_diagnosis + Sex + race, ethnicity + ISS_at_MMdx + HCT_ever + regimen_categoryVCD_1 + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
<!-- # tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!-- tbl2 <- coxph(Surv(time = germline_patient_data$month_at_os,  -->
<!--              event = germline_patient_data$os_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + Sex + race, ethnicity + CH_status_no_indel,  -->
<!--              data =  germline_patient_data) %>% -->
<!--   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data$month_at_os,  -->
<!-- #              event = germline_patient_data$os_event) ~  -->
<!-- #                MMA_germline_results + age_at_diagnosis + Sex + race, ethnicity + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
<!-- # tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!-- #          CH_status_no_indel) %>%  -->
<!-- #   tbl_uvregression(method = survival::coxph,  -->
<!-- #                    y = (Surv(time = germline_patient_data$month_at_progression_drug,  -->
<!-- #                              event = germline_patient_data$drug_progression_event)), -->
<!-- #                    exponentiate = TRUE) %>%  -->
<!-- #   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") %>% -->
<!-- #   bold_labels() %>% italicize_levels() -->
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data$month_at_progression_drug,  -->
<!-- #              event = germline_patient_data$drug_progression_event) ~  -->
<!-- #                MMA_germline_results_cat + age_at_diagnosis + Sex + race, ethnicity + calculated_CrCl +  -->
<!-- #                bmi_germline_results + b12_germline_results + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data) %>% -->
...
<!-- ## Stratified in Male -->
<!-- ```{r, fig.height = 7} -->
<!-- germline_patient_data_male <- germline_patient_data %>%  -->
<!--   filter(Sex == "Male") -->
<!-- ``` -->
<!-- <div class = "row"> -->
<!-- <div class = "col-md-6"> -->
<!-- ```{r, fig.height = 7} -->
<!-- ggsurvplot(survfit(Surv(month_at_os, os_event) ~ MMA_germline_results_cat,  -->
<!--                    data=germline_patient_data_male %>%  -->
<!--                      filter(Sex == "Male")), -->
<!--            title = "OS Analysis", -->
<!--            font.main = c(20, "bold", "black"), -->
<!--            font.x = c(18, "bold", "black"), -->
...
<!-- tbl1 <- germline_patient_data_male %>%  -->
<!--   select(MMA_germline_results_cat,  -->
...
<!--          CH_status_no_indel) %>%  -->
<!--   tbl_uvregression(method = survival::coxph,  -->
<!--                    y = (Surv(time = germline_patient_data_male$month_at_os,  -->
<!--                              event = germline_patient_data_male$os_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") %>% -->
<!--   bold_labels() %>% italicize_levels() -->
<!-- tbl2 <- coxph(Surv(time = germline_patient_data_male$month_at_os,  -->
<!--              event = germline_patient_data_male$os_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + CrCl_interpretation +  -->
<!--                bmi_germline_results + b12_germline_results + CH_status_no_indel,  -->
<!--              data =  germline_patient_data_male) %>% -->
<!--   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data_male$month_at_os,  -->
<!-- #              event = germline_patient_data_male$os_event) ~  -->
<!-- #                MMA_germline_results + age_at_diagnosis + race, ethnicity + calculated_CrCl +  -->
<!-- #                bmi_germline_results + b12_germline_results + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data_male) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data_male$month_at_os,  -->
<!-- #              event = germline_patient_data_male$os_event) ~  -->
<!-- #                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + ISS_at_MMdx + HCT_ever + regimen_categoryVCD_1 + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data_male) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
<!-- # tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!-- tbl2 <- coxph(Surv(time = germline_patient_data_male$month_at_os,  -->
<!--              event = germline_patient_data_male$os_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + CH_status_no_indel,  -->
<!--              data =  germline_patient_data_male) %>% -->
<!--   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data_male$month_at_os,  -->
<!-- #              event = germline_patient_data_male$os_event) ~  -->
<!-- #                MMA_germline_results + age_at_diagnosis + race, ethnicity + CH_status_no_indel,  -->
<!-- #              data =  germline_patient_data_male) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
<!-- # tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!--          CH_status_no_indel) %>%  -->
<!--   tbl_uvregression(method = survival::coxph,  -->
<!--                    y = (Surv(time = germline_patient_data_male$month_at_progression_drug,  -->
<!--                              event = germline_patient_data_male$drug_progression_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") %>% -->
<!--   bold_labels() %>% italicize_levels() -->
<!-- tbl2 <- coxph(Surv(time = germline_patient_data_male$month_at_progression_drug,  -->
<!--              event = germline_patient_data_male$drug_progression_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + CrCl_interpretation +  -->
<!--                bmi_germline_results + b12_germline_results + CH_status_no_indel,  -->
<!--              data =  germline_patient_data_male) %>% -->
...
<!-- ```{r, fig.height = 7} -->
<!-- germline_patient_data_female <- germline_patient_data %>%  -->
<!--   filter(Sex == "Female") -->
<!-- ``` -->
<!-- <div class = "row"> -->
<!-- <div class = "col-md-6"> -->
<!-- ```{r, fig.height = 7} -->
<!-- ggsurvplot(survfit(Surv(month_at_os, os_event) ~ MMA_germline_results_cat,  -->
<!--                    data=germline_patient_data_female), -->
<!--            title = "OS Analysis", -->
<!--            font.main = c(20, "bold", "black"), -->
<!--            font.x = c(18, "bold", "black"), -->
<!--            font.y = c(18, "bold", "black"), -->
...
<!-- tbl1 <- germline_patient_data_female %>%  -->
<!--   select(MMA_germline_results_cat,  -->
<!--          age_at_diagnosis, race, ethnicity, -->
<!--          CrCl_interpretation,  -->
<!--          bmi_germline_results, b12_germline_results, -->
<!--          CH_status) %>%  -->
<!--   tbl_uvregression(method = survival::coxph,  -->
<!--                    y = (Surv(time = germline_patient_data_female$month_at_os,  -->
<!--                              event = germline_patient_data_female$os_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") %>% -->
<!--   bold_labels() %>% italicize_levels() -->
<!-- tbl2 <- coxph(Surv(time = germline_patient_data_female$month_at_os,  -->
<!--              event = germline_patient_data_female$os_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + CrCl_interpretation +  -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data_female$month_at_os,  -->
<!-- #              event = germline_patient_data_female$os_event) ~  -->
<!-- #                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + ISS_at_MMdx + HCT_ever + regimen_categoryVCD_1 + CH_status,  -->
<!-- #              data =  germline_patient_data_female) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
<!-- # tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!-- tbl2 <- coxph(Surv(time = germline_patient_data_female$month_at_os,  -->
<!--              event = germline_patient_data_female$os_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + CH_status,  -->
<!--              data =  germline_patient_data_female) %>% -->
<!--   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
...
<!-- # tbl2 <- coxph(Surv(time = germline_patient_data_female$month_at_os,  -->
<!-- #              event = germline_patient_data_female$os_event) ~  -->
<!-- #                MMA_germline_results + age_at_diagnosis + race, ethnicity + CH_status,  -->
<!-- #              data =  germline_patient_data_female) %>% -->
<!-- #   tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05) -->
<!-- #  -->
<!-- # tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**")) -->
...
<!--          CH_status) %>%  -->
<!--   tbl_uvregression(method = survival::coxph,  -->
<!--                    y = (Surv(time = germline_patient_data_female$month_at_progression_drug,  -->
<!--                              event = germline_patient_data_female$drug_progression_event)), -->
<!--                    exponentiate = TRUE) %>%  -->
<!--   bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") %>% -->
<!--   bold_labels() %>% italicize_levels() -->
<!-- tbl2 <- coxph(Surv(time = germline_patient_data_female$month_at_progression_drug,  -->
<!--              event = germline_patient_data_female$drug_progression_event) ~  -->
<!--                MMA_germline_results_cat + age_at_diagnosis + race, ethnicity + CrCl_interpretation +  -->
<!--                bmi_germline_results + b12_germline_results + CH_status,  -->
<!--              data =  germline_patient_data_female) %>% -->
...
germline_patient_data %>%
  # mutate(egfr_germline_results = as.numeric(egfr_germline_results)) %>%
  # left_join(., germline_patient_data, by = c("patient_id" = "SLID_germline")) %>%
  ggplot(aes(x= calculated_CrCl, y= MMA_germline_results))+
  geom_point()+
  geom_smooth(method = "lm") +
  stat_cor(size=5)+
  stat_regline_equation(label.x = 135, size=5)+
  theme_classic()
...
germline_patient_data %>%
  ...
  # left_join(., germline_patient_data, by = c("patient_id" = "SLID_germline")) %>%
  ggplot(aes(x= b12_germline_results, y= MMA_germline_results))+
  geom_point()+
  geom_smooth(method = "lm") +
  stat_cor(size=5)+
  stat_regline_equation(label.x = 1300, size=5)+
  theme_classic()
```
...
# germline_patient_data_female <- germline_patient_data_female %>% 
#   mutate(MMA_germline_results_cat = case_when(
#     MMA_germline_results_cat == "≤0.4 ~ normal"    ~ 0,
#     MMA_germline_results_cat == ">0.4 ~ high"      ~ 1
#   ))
# germline_patient_data_male <- germline_patient_data_male %>% 
#   mutate(MMA_germline_results_cat = case_when(
#     MMA_germline_results_cat == "≤0.4 ~ normal"    ~ 0,
#     MMA_germline_results_cat == ">0.4 ~ high"      ~ 1
#   ))

tbl1_1 <- glm(MMA_germline_results_cat ~ age_at_diagnosis + race + ethnicity + CH_status_no_indel,
             data = germline_patient_data) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
                         ) %>% 
           modify_spanning_header(everything() ~ paste("Association with MMA in overall data")) %>% 
  bold_labels() %>% bold_p(t = .05)
tbl1_1
tbl2_1 <- glm(MMA_germline_results_cat ~ age_at_diagnosis + race + ethnicity + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status_no_indel,
             data = germline_patient_data) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
  ...
  bold_labels() %>% bold_p(t = .05)
tbl2_1
# tbl1_2 <- glm(MMA_germline_results_cat ~ age_at_diagnosis + race, ethnicity + CH_status,
#              data = germline_patient_data_female) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
#                          ) %>% 
...
#   bold_labels() %>% bold_p(t = .05)
# tbl2_2 <- glm(MMA_germline_results_cat ~ age_at_diagnosis + race, ethnicity + calculated_CrCl + 
#                bmi_germline_results + b12_germline_results + CH_status,
#              data = germline_patient_data_female) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
...
#   bold_labels() %>% bold_p(t = .05)
# 
# tbl1_3 <- glm(MMA_germline_results_cat ~ age_at_diagnosis + race, ethnicity + CH_status,
#              data = germline_patient_data_male) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
#                          ) %>% 
...
#   bold_labels() %>% bold_p(t = .05)
# tbl2_3 <- glm(MMA_germline_results_cat ~ age_at_diagnosis + race, ethnicity + calculated_CrCl + 
#                bmi_germline_results + b12_germline_results + CH_status,
#              data = germline_patient_data_male) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
...
# tbl_merge(list(tbl1_1, tbl1_2, tbl1_3), tab_spanner = c("Overall", "Female", "Male"))
# tbl_merge(list(tbl2_1, tbl2_2, tbl2_3), tab_spanner = c("Overall", "Female", "Male"))
```

## Continuous
```{r}
tbl1_1 <- glm(MMA_germline_results ~ age_at_diagnosis + race + ethnicity + CH_status_no_indel,
             data = germline_patient_data) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
                         ) %>% 
  ...
  bold_labels() %>% bold_p(t = .05)
tbl1_1
tbl2_1 <- glm(MMA_germline_results ~ age_at_diagnosis + race + ethnicity + calculated_CrCl + 
               bmi_germline_results + b12_germline_results + CH_status_no_indel,
             data = germline_patient_data) %>%
          tbl_regression(exponentiate = TRUE#,
                         # label = list(`data[[i]]` ~ i)
  ...
  bold_labels() %>% bold_p(t = .05)
tbl2_1
# tbl1_2 <- glm(MMA_germline_results ~ age_at_diagnosis + race, ethnicity + CH_status,
#              data = germline_patient_data_female) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
#                          ) %>% 
#            modify_spanning_header(everything() ~ paste("Association with MMA in female data")) %>% 
#   bold_labels() %>% bold_p(t = .05)
# tbl2_2 <- glm(MMA_germline_results ~ age_at_diagnosis + race, ethnicity + calculated_CrCl + 
#                bmi_germline_results + b12_germline_results + CH_status,
#              data = germline_patient_data_female) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
...
#   bold_labels() %>% bold_p(t = .05)
# 
# tbl1_3 <- glm(MMA_germline_results ~ age_at_diagnosis + race, ethnicity + CH_status,
#              data = germline_patient_data_male) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
#                          ) %>% 
#            modify_spanning_header(everything() ~ paste("Association with MMA in male data")) %>% 
#   bold_labels() %>% bold_p(t = .05)
# tbl2_3 <- glm(MMA_germline_results ~ age_at_diagnosis + race, ethnicity + calculated_CrCl + 
#                bmi_germline_results + b12_germline_results + CH_status,
#              data = germline_patient_data_male) %>%
#           tbl_regression(exponentiate = TRUE#,
#                          # label = list(`data[[i]]` ~ i)
