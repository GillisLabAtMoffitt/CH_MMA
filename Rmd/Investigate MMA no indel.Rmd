---
title: "Investigate MMA = CH no indel"
...
# Load MRN
IDs_list <- readxl::read_xlsx("MMA_avatar_lab_report.xlsx", sheet = "IDs list") %>% 
  mutate(PATIENT_ID = as.character(PATIENT_ID)) %>% 
  mutate(MRN = as.character(MRN))

# Load EGFR and more
creatinine <- readxl::read_xlsx("MMA_avatar_lab_report_V2.xlsx", sheet = "Updated_Lab_Creatinine") %>% 
  left_join(IDs_list, .,
            by = "PATIENT_ID") %>% 
  filter(str_detect(LAB_RESULT, "\\d+ ?")) %>% 
  # mutate(LAB_RESULT = as.numeric(LAB_RESULT)) %>% 
  filter(!is.na(LAB_RESULT) & SPECIMEN_TYPE == "Blood")
bmi <- readxl::read_xlsx("MMA_avatar_lab_report.xlsx", sheet = "BMI") %>% 
  left_join(IDs_list, .,
            by = "PATIENT_ID") %>% 
  filter(str_detect(VITALS_VALUE, "\\d+ ?"))
height <- readxl::read_xlsx("MMA_avatar_lab_report.xlsx", sheet = "Height") %>% 
...
weight <- readxl::read_xlsx("MMA_avatar_lab_report.xlsx", sheet = "Weight") %>% 
  filter(str_detect(VITALS_VALUE, "\\d+ ?") & 
...
#   filter(str_detect(LAB_RESULT, "\\d+ ?")) %>% 
#   mutate(MRN = as.character(MRN))
# egfr_Yifen <- readxl::read_xlsx("MMA_avatar_lab_report_V2.xlsx", sheet = "Updated_Lab_eGFR") %>% 
#   left_join(IDs_list, .,
#             by = "PATIENT_ID") %>% 
#   filter(str_detect(LAB_RESULT, "\\d+ ?"))
b12 <- readxl::read_xlsx("Vit b12 in Avatar patients.xlsx") %>% 
  filter(!is.na(LAB_RESULT)) %>% 
  mutate(MRN = as.character(MRN))
b12_Yifen <- readxl::read_xlsx("MMA_avatar_lab_report.xlsx", sheet = "Lab_B12") %>% 
  left_join(IDs_list, .,
            by = "PATIENT_ID") %>% 
  filter(!is.na(LAB_RESULT))
# MMA_2 <- readxl::read_xlsx("MMA_avatar_lab_report.xlsx", sheet = "Lab_MMA") %>% 
#   left_join(IDs_list, .,
#             by = "PATIENT_ID") %>% 
#   filter(!is.na(LAB_RESULT))
```
...
# Females: IBW = 45.5 kg + 2.3 kg for each inch over 5 feet.
  # Need height and weight closest to creatinine
  left_join(., height %>%
              select(MRN, VITALS_VALUE, VITALS_DTM),
            by = "MRN") %>%
  # Create var closest height results to germline
  ...
  mutate(adjusted_weight = ((weight_at_creatinine - ideal_weight) * 0.40) + ideal_weight
  ) %>%
  # Calculate CrCl
  mutate(CrCl = case_when(
    Sex == "Female"                   ~ ((
      ((140 - age_at_creatinine) * weight_at_creatinine) /
  ...
  mutate(adjusted_CrCl1 = case_when(
...
tbl1 <- patients_clean_tumor %>% 
  select(MMA_germline_results_cat, 
         Age_at_MMonly_diagnosis, Sex, raceeth,
         egfr_germline_results, bmi_germline_results, b12_germline_results, 
         CH_status_no_indel) %>% 
...
tbl2 <- coxph(Surv(time = patients_clean_tumor$month_at_os, 
             event = patients_clean_tumor$os_event) ~ 
               MMA_germline_results_cat + Age_at_MMonly_diagnosis + Sex + raceeth + egfr_germline_results + bmi_germline_results + b12_germline_results + CH_status_no_indel, 
             data =  patients_clean_tumor) %>%
...
tbl2 <- coxph(Surv(time = patients_clean_tumor_male$month_at_os, 
             event = patients_clean_tumor_male$os_event) ~ 
               MMA_germline_results_cat + Age_at_MMonly_diagnosis + raceeth + egfr_germline_results + bmi_germline_results + b12_germline_results + CH_status_no_indel, 
             data =  patients_clean_tumor_male) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
...
tbl2 <- coxph(Surv(time = patients_clean_tumor_female$month_at_os, 
             event = patients_clean_tumor_female$os_event) ~ 
               MMA_germline_results_cat + Age_at_MMonly_diagnosis + raceeth + egfr_germline_results + bmi_germline_results + b12_germline_results + CH_status, 
             data =  patients_clean_tumor_female) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
...
germline_patient_data %>% 
  mutate(egfr_germline_results = as.numeric(egfr_germline_results)) %>% 
  # left_join(., patients_clean_tumor, by = c("patient_id" = "SLID_germline")) %>% 
  ggplot(aes(x= MMA_germline_results, y= egfr_germline_results))+
  geom_point()+
  geom_smooth(method = "lm") +
  stat_cor(size=5)+ 
  stat_regline_equation(label.x = 1, size=5)+
